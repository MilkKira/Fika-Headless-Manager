...
public partial class DashboardPage : Page, INotifyPropertyChanged
{
    ...
    private static readonly TimeSpan OverlayOpenDuration = TimeSpan.FromMilliseconds(180);
    private static readonly TimeSpan OverlayCloseDuration = TimeSpan.FromMilliseconds(140);
    ...
    private async void DashboardPage_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        if (_window is not null && !_closingSubscribed)
        {
    ...
    private void CancelEditor_Click(object sender, RoutedEventArgs e) => HideOverlay(EditorOverlay);
    ...
    {
        var sequence = ++_overlayAnimationVersion;
        overlay.BeginAnimation(OpacityProperty, null);
        overlay.Opacity = 0;
        overlay.Visibility = Visibility.Visible;

        var content = overlay.Child as FrameworkElement;
        if (content is not null)
        {
            content.RenderTransformOrigin = new Point(0.5, 1);
            var transform = content.RenderTransform as TranslateTransform ?? new TranslateTransform();
            transform.BeginAnimation(TranslateTransform.YProperty, null);
            transform.Y = 10;
            content.RenderTransform = transform;

            var slideUp = new DoubleAnimation(10, 0, OverlayOpenDuration)
            {
                EasingFunction = new CubicEase { EasingMode = EasingMode.EaseOut }
            };
            slideUp.Completed += (_, _) =>
            {
                if (sequence == _overlayAnimationVersion)
                {
                    transform.BeginAnimation(TranslateTransform.YProperty, null);
                    transform.Y = 0;
                }
        ...
        var fadeIn = new DoubleAnimation(0, 1, OverlayOpenDuration)
        {
            EasingFunction = new CubicEase { EasingMode = EasingMode.EaseOut }
        };
    ...
    private void HideOverlay(Border overlay)
    {
        ...
        {
            content.RenderTransformOrigin = new Point(0.5, 1);
            transform = content.RenderTransform as TranslateTransform ?? new TranslateTransform();
            transform.BeginAnimation(TranslateTransform.YProperty, null);
            transform.Y = 0;
            content.RenderTransform = transform;
        }

        var fadeOut = new DoubleAnimation(1, 0, OverlayCloseDuration)
        {
        ...
        if (transform is not null)
        {
            var slideDown = new DoubleAnimation(0, 10, OverlayCloseDuration)
            {
                EasingFunction = new CubicEase { EasingMode = EasingMode.EaseIn }
            };
    ...
    private void ShowWarning(string message) => ShowMessage(message, MessageBoxImage.Warning);

    private void ShowError(string message) => ShowMessage(message, MessageBoxImage.Error);

    private void ShowMessage(string message, MessageBoxImage image)
    {
        if (_window is not null)
        {
            MessageBox.Show(_window, message, "Fika 无头管理器", MessageBoxButton.OK, image);
        }
        else
        {
            MessageBox.Show(message, "Fika 无头管理器", MessageBoxButton.OK, image);
        }
    }
    ...
    private async void Window_Closing(object? sender, CancelEventArgs e)
    {
        ...
        var window = sender as Window ?? _window ?? Window.GetWindow(this);
